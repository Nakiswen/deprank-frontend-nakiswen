  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Show failure modal
        setErrorMessage(result.message || 'Failed to claim reward. Please try again later.');
      ...
      {showSuccess && (
        <div className="w-full max-w-md mx-auto bg-white/90 backdrop-blur-lg rounded-2xl shadow-xl border border-gray-200 overflow-hidden mt-8">
          <div className="p-8 text-center">
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-2">Claim Successful</h2>
            <p className="text-gray-600 mb-6">You have successfully claimed rewards for {dependency}</p>
            <button
      ...
      {!showSuccess && (
        <div className="w-full max-w-md mx-auto bg-white/90 backdrop-blur-lg rounded-2xl shadow-xl border border-gray-200 overflow-hidden transition-all duration-300 hover:shadow-2xl">
          <div className="px-8 py-6 border-b border-gray-200 bg-gray-50/80">
            <h1 className="text-2xl font-bold text-gray-900">Claim Contribution Rewards</h1>
          ...
          <div className="p-8">
            <div className="mb-6">
              <p className="text-gray-700 mb-4">
                As a contributor to {dependency}, you can claim rewards for your contribution. Please enter your wallet address to claim.
              </p>
                ...
                <div className="flex">
                  ...
                  <div className="ml-3">
                    <p className="text-sm text-yellow-700">
                      To prevent abuse, you need to verify your identity with GitHub before claiming. Please ensure you're logged in.
                    </p>
                    
                    {!isLoggedIn && (
                      <div className="mt-2">
                        <div className="text-sm font-medium text-red-600">
                          You are not currently logged in. Please log in with your GitHub account first.
                        </div>
                      </div>
                    )}
                    ...
                    {isLoggedIn && (
                      <div className="mt-2 text-sm font-medium text-green-600">
                        Logged in with GitHub account: {session?.user?.name || session?.user?.username}
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div className="flex justify-end">
              ...
              >
                ...
                ) : !isLoggedIn ? 'Please Log in with GitHub First' : 'Claim Rewards'}
              </button>
            </div>
          </div>
        </div>
