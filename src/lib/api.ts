...
// Dependency details response data
 ...
 */
export async function checkClaimStatus(org: string, repo: string, dependency: string): Promise<ApiResponse<ClaimStatusData>> {
  try {
    // Simulate API request delay
    await delay(500);
    ...
    const claimStatus = allClaimStatus[claimKey] || { claimed: false, lastChecked: new Date().toISOString() };
 ...
 */
export async function claimReward(org: string, repo: string, dependency: string, walletAddress: string): Promise<ApiResponse<ClaimRewardData>> {
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : 'Failed to claim reward, please try again later';
